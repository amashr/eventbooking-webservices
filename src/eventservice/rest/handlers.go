...
func (eh *eventServiceHandler) newEventHandler(w http.ResponseWriter, r *http.Request) {
    ...
    if nil != err {
        w.WriteHeader(500)
        fmt.Fprintf(w, "error occured while persisting event %d %s",id, err)
        return
		}
...
func () newLocationHandler(w http.ResponseWriter, r *http.Request) {
	...
	err := json.NewEncoder(r.Body).Decode(&location)
	if err != nil {
		w.WriteHeader(400)
		fmt.Fprintf(w, "request body could not be unserialized to location: %s", err)
		return
	}
	persistence, err := eh.dbhandler.AddLocation(location)
	if err != nil {
		w.WriteHeader(500)
